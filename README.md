...
## How does it work?
WCTC scans the working directory for files with extension(s) you specify with the `-f` option (defaults to every file).
...
Then a C++ header-like file will be generated containing `const uint8_t` arrays for each file processed and outputs it to a file name specified by `-o` (defaults to "webFiles.h).
